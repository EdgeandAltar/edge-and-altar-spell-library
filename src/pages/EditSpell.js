...
// Slug helper (same behavior as your Admin file)
const slugify = (text = "") =>
  text
    .toLowerCase()
    .trim()
...
function EditSpell() {
  ...
  // UI state uses camelCase (to keep your JSX unchanged)
  ...
  const fetchSpell = async () => {
    ...
    try {
      // Optional: ensure user is logged in (and your RLS admin policy will enforce edit rights)
      const { data: u } = await supabase.auth.getUser();
      if (!u?.user) {
        setError("You must be signed in to edit spells.");
        setLoading(false);
        return;
      }
      ...
      console.error("[EditSpell] fetch error:", err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data: u } = await supabase.auth.getUser();
      if (!u?.user) throw new Error("You must be signed in to edit spells.");
      ...
      const payload = {
        ...
        seasonal_tags: safeSeasonalTags,
        tags: safeTags,
        slug: slugify(formData.title) || null,

        // ✅ If you created updated_at, keep this. If not, delete this line.
      ...
      console.error("[EditSpell] update error:", err);
      setError("Error updating spell: " + (err?.message || "Unknown error"));
    } finally {
      setSaving(false);
    }
  };
